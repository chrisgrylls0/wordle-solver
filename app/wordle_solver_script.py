...
ENG_DICT_PATH = Path(DICTIONARIES_PATH, "words_alpha.txt")
FIVE_LETTER_WORD_FREQ = Path(DICTIONARIES_PATH,"6_letter_word_freq.json")
...
def valid_result(result):
    ...
    for letter in result:
        if letter not in valid_results:
            return False
...
def update_valid_letters(guess, result, letters, all_letters):
    ...
    for i, letter in enumerate(guess):
        if result[i] == "G":
            ...
            all_letters["green"] = all_letters["green"].union(
                letters["green"]
            )  # update full list
        elif result[i] == "Y":
            letters["yellow"].add((letter, i))
            all_letters["yellow"] = all_letters["yellow"].union(
                letters["yellow"]
            )  # update full list
            ...
            try:
                yellow_letters_only, _ = zip(
                    *letters["yellow"]
                )  # try except incase there are no letters in yellow yet
            except ValueError:
                yellow_letters_only = []
            ...
            if not letter in yellow_letters_only:
                ...
                duplicate_letters = [
                    l for l in string.ascii_lowercase if guess.count(l) >= 2
                ]
                grey_rem = set()
                if len(duplicate_letters) != 0:  # only if there is a duplicate letter
                    ...
                    try:
                        grey_rem.remove(
                            (letter, i)
                        )  # remove the current (we only want the other letter's position here, since we'll exclude it later)
                    except KeyError:
                        pass  # try-except, incase we try to remove nothing
                ...
                try:
                    grey_add.remove(
                        grey_add.intersection(all_letters["green"])
                    )  # remove any in green
                except KeyError:
                    pass  # try-except, incase we try to remove nothing

                letters["grey"] = letters["grey"].union(grey_add)
                all_letters["grey"] = all_letters["grey"].union(
                    letters["grey"]
                )  # update full list
    ...
    try:
        all_letters["grey"].remove(
            all_letters["grey"].intersection(all_letters["green"])
        )  # remove any in green
    except KeyError:
        pass  # try-except, incase we try to remove nothing
    try:
        letters["grey"].remove(
            letters["grey"].intersection(all_letters["green"])
        )  # remove any in green
    except KeyError:
        pass  # try-except, incase we try to remove nothing
    ...
    try:
        all_letters["green"].remove(
            all_letters["green"].intersection(all_letters["grey"])
        )  # remove any in green
    except KeyError:
        pass  # try-except, incase we try to remove nothing
    try:
        letters["green"].remove(
            letters["green"].intersection(all_letters["grey"])
        )  # remove any in green
    except KeyError:
        pass  # try-except, incase we try to remove nothing
...
def assess_words(wl, letter_set, l_type):
    temp_word_list = (
        wl.copy()
    )  # create temp copy so we're not iterating and modifying the same list
    for word in temp_word_list:
        for letter, i in letter_set:
            if (
                l_type == "G"
            ):  # remove word if letter does *not* occur at given position
                if word[i] == letter:
                    pass  # keep it
                else:
                    try:
                        ...
                        pass  # try-except, since word may have already been removed when iterating through letters
...
def reduce_word_list(
    result, word_list, letters, printout=False
):  # assess_words() inside this
    # assess word_list (for current iteration)
    # order: grey, green, yellow
    # for grey letters
    if printout:
        print("After eliminating words based on:")
    if "-" in result:
        word_list = assess_words(word_list, letters["grey"], "-")
        if printout:
            print(f"    new GREY letters, remains: {len(word_list)} words")

    # for green letters
    if "G" in result:
        word_list = assess_words(word_list, letters["green"], "G")
        if printout:
            print(f"    new GREEN letters, remains: {len(word_list)} words")

    # for yellow letters
    if "Y" in result:
        word_list = assess_words(word_list, letters["yellow"], "Y")
        if printout:
            print(f"    new YELLOW letters, remains: {len(word_list)} words")
...
class Word_Counted:
    def __init__(self, word, letter_occurrences):
        self.word = word
        self.letter_sum = sum(
            [letter_occurrences[i][l] for i, l in enumerate(self.word)]
        )
...
def order_by_likelihood(word_list, json_filename, printout=False):
    ...
    if printout:
        ...
        if sorted_in_word_freq == []:
            ...
            pprint.pprint(in_word_freq, sort_dicts=False)
...
def best_guess_to_whittle_down(word_list, all_five_letter_words):
    ...
    # order this dict
    least_common_letters = sorted(
        letter_count_dict_rems, key=letter_count_dict_rems.get
    )
...
def main(WORD_LEN, eng_dict_path, NUM_WORDS_TO_RETURN):
    ...
    # create sets
    all_letters = {"grey": set(), "yellow": set(), "green": set()}

    while attempt_no < NUM_ATTEMPTS:

        guess = " "
        letters = {"grey": set(), "yellow": set(), "green": set()}
        ...
        # ask for word input
        while (
            not guess.isalpha() or len(guess) != WORD_LEN
        ):  # make sure '-' isn't in the word, it should only be letters, and its the correct length
        ...
        if len(top_words) <= 15:
            top_words = order_by_likelihood(
                top_words, FIVE_LETTER_WORD_FREQ, printout=True
            )

        print(
            f"Next {len(top_words[:NUM_WORDS_TO_RETURN])} best words are: {top_words[:NUM_WORDS_TO_RETURN]}"
        )
        ...
        if result.count("G") >= WORD_LEN - 2:
            ...
            best_guess_words = best_guess_to_whittle_down(
                word_list, all_five_letter_words
            )
    ...
    if solved:
        print()
        print("Congratulations for solving today's wordle!")
